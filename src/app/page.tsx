...
// 100% Verified Kalshi Primary Key Contracts
const KALSHI_KEYED_CONTRACTS: PlayerContract[] = [
  {
...
export default function Page() {
  ...
  const topTen = useMemo(() => {
    return [...KALSHI_KEYED_CONTRACTS].sort((a, b) => b.edgeVal - a.edgeVal).slice(0, 10);
  }, []);
  ...
  const displayedPlayers = useMemo(() => {
    if (searchQuery.trim().length > 0) {
      return KALSHI_KEYED_CONTRACTS.filter((p) => {
      ...
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        <header className="border-b border-zinc-800 pb-4 flex flex-wrap justify-between items-center gap-4">
          ...
          <div className="text-xs bg-zinc-900 border border-zinc-800 px-3 py-1.5 rounded text-zinc-400">
            ENGINE: <span className="text-emerald-400 font-bold">KALSHI KEYED</span> | CONTRACTS: <span className="text-white">{KALSHI_KEYED_CONTRACTS.length} SYNCED</span>
          </div>
        </header>
